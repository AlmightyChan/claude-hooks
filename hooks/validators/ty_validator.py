...
Runs `uvx ty check` for type checking on single Python files.
...
def main() -> None:
    ...
    # Run uvx ty check on the single file (--ignore unresolved-import inline)
    logger.info(f"Running: uvx ty check --ignore unresolved-import {file_path}")
    try:
