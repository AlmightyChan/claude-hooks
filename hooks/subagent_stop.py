...
# ///
"""
Subagent stop hook: Subagent completion logging and transcript export.
Logs subagent stop events and optionally exports chat transcript.
"""
...
def export_transcript(session_id: str, subagent_id: str, transcript_path: str | None) -> bool:
    """Export the subagent transcript to chat.jsonl."""
    ...
    try:
        ...
        # Add as a subagent session entry
        session_entry = {
...
def main() -> int:
    ...
    exported = False
    if args.chat:
        exported = export_transcript(session_id, subagent_id, transcript_path)
    ...
    # Save log
    append_jsonl(get_log_dir() / "subagent_stop.jsonl", log_entry)

    # Unified event stream
    append_event("SubagentStop", session_id, {"subagent_id": subagent_id, "subagent_type": subagent_type})
